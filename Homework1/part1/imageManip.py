...
def rotate2d(point, theta):
    ...
    point = np.dot(np.asarray([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]]), point)
...
def rotate_image(input_image, theta):
    ...
    ## YOUR CODE HERE
    input_rows_mean = input_rows / 2
    input_cols_mean = input_cols / 2
    for i in range(input_rows):
        for j in range(input_cols):
            new_row, new_col = i - input_rows_mean, j - input_cols_mean
            new_row, new_col = rotate2d(np.array([new_row, new_col]), theta).tolist()
            new_row, new_col = new_row + input_rows_mean, new_col + input_cols_mean
            new_row, new_col = round(new_row), round(new_col)
            if new_row < 0 or new_col < 0 or new_row >= input_rows or new_col >= input_cols:
                continue
            output_image[i, j, :] = input_image[new_row, new_col, :]
    ### END YOUR CODE
